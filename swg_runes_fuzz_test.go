...
// swg_runes_fuzz_test.go runs native Go fuzzing against
// SmithWatermanGotohScoreRunes — the rune-aware variant of the
// Smith-Waterman 1981 / Gotoh 1982 local-alignment score. Pattern
// mirrors levenshtein_runes_fuzz_test.go.
//
// Properties:
//
...
// FuzzSmithWatermanGotohScoreRunes asserts panic-free + score in
...
func FuzzSmithWatermanGotohScoreRunes(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, a, b string) {
		got := fuzzymatch.SmithWatermanGotohScoreRunes(a, b)
		if math.IsNaN(got) {
			t.Fatalf("SmithWatermanGotohScoreRunes(%q, %q) = NaN", a, b)
		}
		if math.IsInf(got, 0) {
			t.Fatalf("SmithWatermanGotohScoreRunes(%q, %q) = Inf", a, b)
		}
		if got < 0.0 || got > 1.0 {
			t.Fatalf("SmithWatermanGotohScoreRunes(%q, %q) = %g; want in [0,1]", a, b, got)
		}
	})
}
