...
// scorer_options_internal_test.go consolidates two concerns that both
// live in package fuzzymatch (not fuzzymatch_test) so they can reach
// unexported scorerConfig state:
//
//  1. Probe helpers (applyOptionForProbe, probeEntryCount, …) used by
//     the test functions below to drive a ScorerOption against a fresh
//     scorerConfig and inspect both the returned error AND the
//     resulting unexported configuration. The probe is the chosen
//     mechanism (per CONTEXT.md §"Claude's Discretion") for plan 08-01
//     to unit-test the option layer in isolation — plan 08-02
//     introduces NewScorer and at that point the probes remain as a
//     thin internal gate for unexported invariants.
//
//  2. Tests covering happy + error paths for every Phase 8 ScorerOption
//     introduced in plan 08-01. These tests live in package fuzzymatch
//     so they can call the probe helpers directly. The file name
//     follows the project's `_internal_test.go` convention for
//     same-package test files (Phase 8.5 Plan 15a Gap 1 rename:
//     was scorer_options_test.go).
//
// Living in package fuzzymatch (not fuzzymatch_test) is the
// conventional Go pattern for exposing package-internal state to
// black-box tests via the _test.go file suffix, which excludes the
// file from production builds. Stdlib testing only — no testify in
// root (per CLAUDE.md and .claude/skills/go-coding-standards/SKILL.md).
...
// caller (the Test* functions below) can then assert on the error via
...
// in a scorerConfig. Used by the Test* functions below to assert
