...
must_haves:
  truths:
    ...
    - "Task 1 ships a TDD canary stub `TestDamerauLevenshteinFull_DiscriminatingVector_Stub` in `damerau_full_discriminator_test.go` that asserts `DamerauLevenshteinFullDistance(\"ca\",\"abc\") == 2 && != 3` — this gates the recurrence at Task 1, BEFORE the full reference-vector suite runs in Task 2"
  artifacts:
    ...
    - path: "damerau_full_discriminator_test.go"
      provides: "Task 1 TDD canary: TestDamerauLevenshteinFull_DiscriminatingVector_Stub asserting Distance(\"ca\",\"abc\") == 2 (NOT 3 — that is OSA's value)"
      contains: "TestDamerauLevenshteinFull_DiscriminatingVector_Stub"
...
<task type="auto" tdd="true">
  <name>Task 1: Implement damerau_full.go (two-row DP + auxiliary tables) and dispatch_damerau_full.go + TDD canary stub test</name>
  <files>damerau_full.go, dispatch_damerau_full.go, damerau_full_discriminator_test.go</files>
  <read_first>
    ...
    - damerau_osa.go (sibling Wave 2 plan output, if landed first — read the OSA recurrence to understand what NOT to do; DL-Full's recurrence returns 2 for "ca"/"abc" while OSA returns 3. If 02-05 has not landed yet, read its plan file 02-05-damerau-levenshtein-osa-PLAN.md §algorithm_specifics for the OSA recurrence sketch.)
...
Create `damerau_full_discriminator_test.go` (package fuzzymatch_test, stdlib testing only — TDD CANARY for Task 1):

This file ships ONE test only. Its sole purpose is to gate the recurrence at Task 1 BEFORE the full reference-vector suite runs in Task 2. A wrong Lowrance-Wagner implementation that returns 3 (collapsing into OSA behaviour) for "ca"/"abc" would otherwise commit silently from Task 1 and only surface at Task 2, wasting Task 2 context budget on debugging.

1. Apache-2.0 header copied from normalise.go lines 1-13.
2. File-level godoc explaining this is the Task 1 TDD canary; the full reference-vector suite lives in `damerau_full_test.go` (Task 2). Reference plan 02-06 §B-1 (revision context).
3. Single test:
       ...
       func TestDamerauLevenshteinFull_DiscriminatingVector_Stub(t *testing.T) {
           ...
           // test fails with got==3, the recurrence collapsed into OSA.
...
After writing all three files: `go build ./... && go vet ./... && bash scripts/verify-license-headers.sh && go test -run TestDamerauLevenshteinFull_DiscriminatingVector_Stub -count=1 ./...`. The stub test MUST pass before this task is considered done — if it fails with got==3, the recurrence is wrong and must be fixed before proceeding to Task 2.
  </action>
  <verify>
    <automated>go build ./... && go vet ./... && bash scripts/verify-license-headers.sh && go test -run TestDamerauLevenshteinFull_DiscriminatingVector_Stub -count=1 ./...</automated>
  </verify>
  <acceptance_criteria>
    ...
    - damerau_full_discriminator_test.go contains the single test `TestDamerauLevenshteinFull_DiscriminatingVector_Stub` and uses no testify.
    - `go build ./...` exits 0; `go vet ./...` exits 0.
    - `go test -run TestDamerauLevenshteinFull_DiscriminatingVector_Stub -count=1 ./...` exits 0 — the recurrence returns 2 for "ca"/"abc" and is NOT collapsed into OSA semantics. THIS IS THE TASK 1 GATE: if the stub fails, fix the recurrence before proceeding to Task 2.
  </acceptance_criteria>
  <behavior>
    - DamerauLevenshteinFullDistance("ab", "ba") == 1
    - DamerauLevenshteinFullDistance("ca", "abc") == 2 (LOCKED discriminating vector — proves Full != OSA; gated at Task 1 by the stub test)
  ...
  <done>
    damerau_full.go, dispatch_damerau_full.go, and damerau_full_discriminator_test.go committed. Package builds. The TDD canary `TestDamerauLevenshteinFull_DiscriminatingVector_Stub` passes — recurrence verified at Task 1 (NOT 3 — that is OSA's value).
  </done>
</task>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - damerau_full_discriminator_test.go (Task 1's TDD canary — TestDamerauLevenshteinFull_DiscriminatingVector_Stub already exists; do NOT duplicate it. The full reference-vector suite goes in damerau_full_test.go below.)
...
2. Tests (note: TestDamerauLevenshteinFull_DiscriminatingVector_Stub already exists in `damerau_full_discriminator_test.go` from Task 1 — do not duplicate it; this file ships the FULL reference-vector suite that builds on the stub):
     ...
     - TestDamerauLevenshteinFull_DiscriminatingVector — full assertion (richer than the Task 1 stub) that DamerauLevenshteinFullDistance("ca","abc") == 2 AND DamerauLevenshteinFullScore("ca","abc") ≈ 0.3333. REFERENCE the OSA-vs-Full divergence in the comment with a citation to RESEARCH.md §DL-OSA vs DL-Full Divergence and ROADMAP success criterion #2.
...
Extend `props_test.go` (APPEND — append after the last function whose name starts with `TestProp_DamerauLevenshteinOSA` if plan 02-05 landed first; otherwise append after the last `TestProp_Hamming*` or `TestProp_Levenshtein*` function — find the last existing `TestProp_*` declaration with grep and append after it):
...
3. Generate via the `assertGoldenStaging` helper defined by plan 02-01 Task 3 (signature LOCKED: `assertGoldenStaging(t *testing.T, relPath string, v any)`; writes to `testdata/golden/_staging/<basename>.json` using the canonical `WriteGoldenFile` discipline). Add `TestGolden_DamerauLevenshteinFull_Staging` to algorithms_golden_test.go (gated on -update). The test calls `assertGoldenStaging(t, "_staging/damerau_full.json", file)` — UNCONDITIONALLY. There is no "if helper exists / else create" branch; plan 02-01 owns the helper definition. Run with `-update` once; commit. Re-run without `-update` and confirm no diff.
...
| T-02-06-05 | Tampering | Discriminating-vector contract weakened (e.g. recurrence drift makes ca/abc return 3, collapsing into OSA behaviour) | mitigate | The Task 1 TDD canary `TestDamerauLevenshteinFull_DiscriminatingVector_Stub` + Task 2's TestDamerauLevenshteinFull_DiscriminatingVector + the BDD scenario + the staging golden file gate on the value 2 (not 3). Four independent enforcement points — including a Task 1 gate that catches recurrence drift BEFORE Task 2 runs; algorithm-correctness-reviewer reviews any recurrence change. |
...
- Confirmation that TestDamerauLevenshteinFull_DiscriminatingVector_Stub (Task 1 canary) AND TestDamerauLevenshteinFull_DiscriminatingVector (Task 2) AND the property tests (especially TriangleInequality) pass.
