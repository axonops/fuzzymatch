...
- **Cross-group bucket strategy — global bucket once, per-pair group-filter (v1.x)** — discovered via `BenchmarkScanCheck_DefaultScorer_10k_CrossGroup` in Plan 09-04 remediation (2026-05-20). The current implementation builds a fresh tokenBucket over `idxA ∪ idxB` per (gi, gj) group-pair → at 500 groups, that's 124,750 bucket rebuilds + ~50M Scorer.Score calls + 1.3B allocations per Check; measured at ~189 s on darwin/arm64 Apple M2 vs 361 ms within-only (~525×, not the spec §12.6 "at most 2×" claim). The v1.x optimisation builds the global tokenBucket once at Check entry, then per group-pair filters candidate membership by group identity — avoids the per-pair rebuild while preserving SCAN-02 correctness equivalence. Out of scope for Phase 9 (would require a third significant Check-body refactor on top of 09-03 + 09-04; the SCAN-02 property test must continue to pass through any future rework). The bench is committed as a v1.x baseline.
