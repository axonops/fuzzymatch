...
- [x] **VALIDATE-01**: Public `fuzzymatch.Validate(a, b string) []Warning` function — single-pass diagnostic returning nil if no warnings apply; safe for concurrent use; never panics; never returns an error (`docs/requirements.md` §11.5)
- [x] **VALIDATE-02**: Public `Warning` struct with `Algorithm AlgoID`, `Kind WarnKind`, `Detail string` fields (`docs/requirements.md` §11.5)
...
| VALIDATE-01 | Phase 8.5 | Complete |
| VALIDATE-02 | Phase 8.5 | Complete |
